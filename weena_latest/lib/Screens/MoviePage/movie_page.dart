...
import 'package:weena_latest/Screens/MoviePage/Add_Episode.dart';
import 'package:weena_latest/Screens/MoviePage/EditeDrama_s/EditDramas.dart';
import 'package:weena_latest/Screens/MoviePage/MoviePageButtons.dart';
import 'package:weena_latest/Screens/MoviePage/Series/ViewEpisodes.dart';
import 'package:weena_latest/Screens/MoviePage/TrailerDialog.dart';
...
class _MoviePageState extends State<MoviePage> {
  ...
  int _views = 0;
  getViewsCount() async {
    int viewsCount = await DatabaseServices.getViews(widget.postModel);
    if (mounted) {
      setState(() {
        _views = viewsCount;
      });
  ...
  getEpisode() async {
    List<PostModel> otherEpisode = await DatabaseServices.getOtherEpisodes(
      widget.postModel,
    );
    if (mounted) {
  ...
  getSeries() async {
    List<SeriesModel> otherSeries = await DatabaseServices.getSeries(
      widget.postModel,
    );
    if (mounted) {
      _series = otherSeries;
  ...
  setupIsMovieOnPlus18() async {
    bool x = await DatabaseServices.setupIsMovieOnPlus18(
            ...
            child: SafeArea(
              child: Column(
                children: [
                  ...
                  const Divider(color: Color.fromARGB(44, 255, 255, 255)),
                  widget.postModel.type == 'Series'
