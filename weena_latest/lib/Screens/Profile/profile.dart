...
import 'package:weena_latest/Widgets/Feed Widgets/FeedContainer.dart';
import 'package:weena_latest/Widgets/PostConatiner.dart';
...
class _ProfileScreensState extends State<ProfileScreen> {
  ...
  getFollowersCount() async {
    _isRefreshing = true;
    int followersCount = await DatabaseServices.followersNumber(
  ...
  getFollowingCount() async {
    _isRefreshing = true;
    int followingCount = await DatabaseServices.followingNumber(
      widget.visitedUserId,
    );
    if (mounted) {
      setState(() {
  ...
  void _genrateColors() async {
    paletteGenerator = await PaletteGenerator.fromImageProvider(
      CachedNetworkImageProvider(
        MyEncriptionDecription.decryptWithAESKey(widget.userModel.coverPicture),
      ),
    );
  }
  ...
  getAllPosts() async {
    _isRefreshing = true;
    List<PostModel> userPosts = await DatabaseServices.getUserPosts(
      widget.visitedUserId,
    );
    if (mounted) {
      setState(() {
        ...
        return Center(
          child: Column(
            children: [
              ...
              Icon(CupertinoIcons.camera_circle, size: 55, color: whiteColor),
              text(
                'هیچ نییە جارێ',
                whiteColor,
                21,
              ...
              SliverAppBar(
                ...
                flexibleSpace: FlexibleSpaceBar(
                  background: GestureDetector(
                    ...
                    child:
                        widget.userModel.coverPicture.isEmpty
                            ? Container(
      ...
      builder: (BuildContext context) {
        return EmergeAlertDialog(
          ...
          emergeAlertDialogOptions: EmergeAlertDialogOptions(
            ...
            title: text(
              "دڵنیای لە شوێن نەکەوتنی ${widget.userModel.name}",
              whiteColor,
              18,
              FontWeight.normal,
              TextDirection.rtl,
              ...
              if (linksModel.facebook.isNotEmpty)
                Padding(
                  ...
                  child: GestureDetector(
                    ...
                    child: socialMediaButton(Colors.blue, FontAwesome.facebook),
                  ),
                )
              else
                const SizedBox(),
  ...
  Widget moreVertonProfile(
    ...
    return PopupMenuButton(
      icon: const Icon(FlutterIcons.more_vert_mdi),
      shadowColor:
          paletteGenerator != null
              ? paletteGenerator!.vibrantColor != null
                  ? paletteGenerator!.vibrantColor!.color
